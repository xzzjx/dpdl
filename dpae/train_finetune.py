...
def extract_xception_features(x):
    from keras.applications.xception import preprocess_input, Xception
    print(x.shape)
    # im_h = x.shape[1]
    im_h = 100
    model = Xception(include_top=False, weights='imagenet', input_shape=(im_h, im_h, 3))
    # if flatten:
    #     add_layer = Flatten()
    # else:
...
def extract_feature(x_train, x_test):
    ...
    e_train = np.zeros([x_train.shape[0], 18432], dtype=np.float32)
    e_test = np.zeros([x_test.shape[0], 18432], dtype=np.float32)
    for i in range(x_train.shape[0]//5000):
        print("the {} 5000 sample.".format(i))
        e_train[5000*i: 5000*(i+1), :] = extract_xception_features(x_train[5000*i: 5000*(i+1)]).reshape(5000, -1)
    for i in range(x_test.shape[0]//5000):
        print("the {} 5000 sample.".format(i))
        e_test[5000*i: 5000*(i+1), :] = extract_xception_features(x_test[5000*i: 5000*(i+1)]).reshape(5000, -1)
